...
//		updateTrueValue(a->right,val);
	...
	int nSamples = 100, nHit = 0,nInv = 0;
...
//		int sample = sampleFromTree(k,a,query_ones);
		int sample = dictionarySample(k,nVertices);
		if (sample==-1) nInv++;
		clock_gettime(CLOCK_MONOTONIC,&finish);
		elapsed += finish.tv_sec - start.tv_sec;
		elapsed += (finish.tv_nsec - start.tv_nsec)/pow(10,9);
		//printf("SAMPLE OBTAINED = %d\n",sample);
	...
	printf("(M=%d,n=%d,m=%d) : TIME TAKEN = %lf,CHI_SQUARE = %lf,ACCURACY = %lf ,MEMBERSHIP = %lf\n",
			nVertices,nNodes,VECT_SIZE,elapsed/nSamples,chi_square,(1.0*nHit)/nSamples, (1.0*nMembership)/nSamples);
